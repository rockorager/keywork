//! Owns tiled workspace tree state, geometry, constraints, and interactive resize state.
//! Window IDs are leaves; internal nodes split available space between child subtrees.

const TiledLayout = @This();
...
pub fn deinit(self: *TiledLayout, allocator: std.mem.Allocator) void {
...
pub fn windowRemoved(self: *TiledLayout, id: types.WindowId) void {
...
pub fn swapWindows(self: *TiledLayout, first: types.WindowId, second: types.WindowId) void {
...
pub fn cancelResize(self: *TiledLayout, resize: Resize) bool {
    const split = self.resizeSplit(resize) orelse return false;
...
fn resizeSplit(self: *TiledLayout, resize: Resize) ?*Split {
...
pub fn setUsableArea(self: *TiledLayout, usable: types.Rect) void {
    self.last_usable = inset(usable, self.outer_gap);
    self.refreshRects();
}

fn refreshRects(self: *TiledLayout) void {
    const root = self.root orelse return;
    self.refreshNode(root, self.last_usable orelse return);
}

fn refreshNode(self: *TiledLayout, node_index: NodeIndex, area: types.Rect) void {
...
fn findLeaf(self: *const TiledLayout, id: types.WindowId) ?NodeIndex {
...
fn extremeLeaf(self: *const TiledLayout, start: NodeIndex, reverse: bool) NodeIndex {
...
fn leafId(self: *const TiledLayout, node_index: NodeIndex) types.WindowId {
    return self.nodes.items[node_index].content.leaf;
}

fn allocateNode(self: *TiledLayout, node: Node) NodeIndex {
...
fn releaseNode(self: *TiledLayout, node_index: NodeIndex) void {
...
fn nodeConstraints(constraints: types.SizeConstraints) TiledLayout.NodeConstraints {
...
fn spanConstraints(minimum: u32, maximum: ?u32) TiledLayout.SpanConstraints {
    return .{
        .minimum = minimum,
...
fn combineNodeConstraints(
    first: TiledLayout.NodeConstraints,
    second: TiledLayout.NodeConstraints,
    split_axis: TiledLayout.Axis,
    preferred_gap: u32,
) TiledLayout.NodeConstraints {
    return .{
...
fn combineSpanConstraints(
    first: TiledLayout.SpanConstraints,
    second: TiledLayout.SpanConstraints,
    divided: bool,
    preferred_gap: u32,
) TiledLayout.SpanConstraints {
...
fn divideConstrained(
    ...
    ratio_percent: u8,
    requested_gap: u32,
    first_constraints: TiledLayout.SpanConstraints,
    second_constraints: TiledLayout.SpanConstraints,
) ?Division {
    const length = if (axis == .horizontal) rect.size.width else rect.size.height;
    if (length < 2) return null;
...
test "nested tiled constraints preserve coverage and visible pass-through" {
    ...
    var layout: TiledLayout = .{ .outer_gap = 0, .inner_gap = 0 };
    defer layout.deinit(std.testing.allocator);

    try layout.windowAdded(std.testing.allocator, first.id, null);
    var plans = try layout.arrange(std.testing.allocator, &.{first}, area);
    plans.deinit(std.testing.allocator);
    try layout.windowAdded(std.testing.allocator, second.id, first.id);
    plans = try layout.arrange(std.testing.allocator, &.{ first, second }, area);
    plans.deinit(std.testing.allocator);
    ...
    plans = try layout.arrange(std.testing.allocator, &.{third}, area);
...
test "tiled resize uses constrained split geometry" {
    ...
    var plans = try layout.arrange(std.testing.allocator, &.{ first, second }, area);
    plans.deinit(std.testing.allocator);
    const resize = layout.beginResize(first.id, 69, 30, 2).?;
    try std.testing.expectEqual(@as(u8, 50), resize.original_split_ratio_percent);
    try std.testing.expectEqual(@as(u8, 70), resize.initial_ratio_percent);
...
test "tiled pointer resize accepts a window edge across a wide gap" {
    const first = input(0, 10);
    const second = input(1, 10);
    var layout: TiledLayout = .{ .outer_gap = 0, .inner_gap = 20 };
    defer layout.deinit(std.testing.allocator);
    try layout.windowAdded(std.testing.allocator, first.id, null);
    try layout.windowAdded(std.testing.allocator, second.id, first.id);
    var plans = try layout.arrange(std.testing.allocator, &.{ first, second }, .{
