//! Generated privileged EXT data-control protocol adapter.
//!
//! This owns wire resources only; DataDevice owns all selection semantics.

const WayringDataControl = @This();
...
const Manager = struct { owner: *WayringDataControl, client: *wayring.server.Client, resource: protocol.ext_data_control_manager_v1.Resource };
const Source = struct { owner: *WayringDataControl, client: *wayring.server.Client, resource: protocol.ext_data_control_source_v1.Resource, id: DataDevice.ControlSourceId };
const Device = struct { owner: *WayringDataControl, client: *wayring.server.Client, resource: protocol.ext_data_control_device_v1.Resource, id: DataDevice.ControlDeviceId };
const Offer = struct { owner: *WayringDataControl, client: *wayring.server.Client, resource: protocol.ext_data_control_offer_v1.Resource, id: DataDevice.ControlOfferId, device: ?*Device, published: bool = false };
...
pub fn init(self: *WayringDataControl, allocator: std.mem.Allocator, protocol_server: *wayring.server.Server, clients: *WayringClients, seat: *WayringSeatAdapter, canonical: *DataDevice, authorized_uid: std.os.linux.uid_t) void {
...
pub fn deinit(self: *WayringDataControl) void {
...
pub fn publish(self: *WayringDataControl) !void {
    std.debug.assert(self.global == null);
    self.global = try self.protocol_server.addGlobalWithOptions(protocol.ext_data_control_manager_v1, 1, WayringDataControl, self, bindManager, .{
        .visibility = .restricted,
    });
}

pub fn unpublish(self: *WayringDataControl) void {
...
fn managerRequest(_: *protocol.ext_data_control_manager_v1.Resource, request: protocol.ext_data_control_manager_v1.Request, value: *Manager) !void {
    switch (request) {
...
fn createSource(self: *WayringDataControl, manager: *Manager, id: u32) !void {
...
fn sourceRequest(_: *protocol.ext_data_control_source_v1.Resource, request: protocol.ext_data_control_source_v1.Request, value: *Source) !void {
    switch (request) {
        .offer => |args| value.owner.canonical.offerControlMime(value.id, args.mime_type) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            error.SourceAlreadyUsed => value.client.postProtocolError(&value.resource.runtime, @intCast(protocol.ext_data_control_source_v1.@"error".invalid_offer), "offer sent after source use"),
            else => {},
        },
...
fn createDevice(self: *WayringDataControl, manager: *Manager, id: u32) !void {
    ...
    try manager.client.materialize(&value.resource.runtime);
    value.id = try self.canonical.createControlDevice(self.clients.id(manager.client) orelse return error.InvalidClient, .{
        .context = value,
...
fn deviceRequest(_: *protocol.ext_data_control_device_v1.Resource, request: protocol.ext_data_control_device_v1.Request, value: *Device) !void {
    switch (request) {
...
fn setSelection(self: *WayringDataControl, device: *Device, object: ?u32, primary: bool) !void {
    ...
    result catch |err| switch (err) {
        ...
        error.SourceAlreadyUsed => device.client.postProtocolError(&device.resource.runtime, @intCast(protocol.ext_data_control_device_v1.@"error".used_source), "data-control source was already used"),
        else => device.client.postImplementationError(&device.resource.runtime, "invalid data-control selection"),
    };
}

fn sourceIdentity(self: *WayringDataControl, client: *wayring.server.Client, object_id: u32) ?DataDevice.ControlSourceId {
...
fn createOffer(self: *WayringDataControl, device: *Device, canonical_id: DataDevice.ControlOfferId) !*Offer {
...
fn prepareChannel(context: *anyopaque, canonical_id: ?DataDevice.ControlOfferId, event_opcode: u16) error{OutOfMemory}!void {
    ...
    const id = canonical_id orelse return device.owner.stageEvent(device.client, &device.resource.runtime, event_opcode, &protocol.ext_data_control_device_v1.event_messages[event_opcode], &.{.{ .object = null }}, 12, &.{});
    ...
    try device.owner.stageEvent(device.client, &device.resource.runtime, 0, &protocol.ext_data_control_device_v1.event_messages[0], &.{.{ .new_id = .{ .typed = offer.resource.id() } }}, 12, &.{offer});
    for (mimes) |mime| {
        ...
        try device.owner.stageEvent(device.client, &offer.resource.runtime, 0, &protocol.ext_data_control_offer_v1.event_messages[0], &.{.{ .string = mime }}, bytes, &.{});
    }
    try device.owner.stageEvent(device.client, &device.resource.runtime, event_opcode, &protocol.ext_data_control_device_v1.event_messages[event_opcode], &.{.{ .object = offer.resource.id() }}, 12, &.{});
}

fn offerRequest(_: *protocol.ext_data_control_offer_v1.Resource, request: protocol.ext_data_control_offer_v1.Request, value: *Offer) !void {
    switch (request) {
        .receive => |args| {
...
pub fn offerRolledBack(context: *anyopaque, id: DataDevice.ControlOfferId) void {
    const self: *WayringDataControl = @ptrCast(@alignCast(context));
...
pub fn offerMimeOffered(context: *anyopaque, id: DataDevice.ControlOfferId, mime: []const u8) void {
    ...
    for (self.offers.items) |offer| if (std.meta.eql(offer.id, id)) {
        ...
        protocol.ext_data_control_offer_v1.@"send:offer"(&offer.resource, mime) catch offer.client.postOutOfMemory(&offer.resource.runtime, "queueing data-control MIME");
        return;
    };
}
...
fn stageEvent(self: *WayringDataControl, client: *wayring.server.Client, resource: *wayring.server.Resource, opcode: u16, descriptor: *const wire.MessageDescriptor, source_values: []const wire.Value, maximum_bytes: usize, offers: []const *Offer) error{OutOfMemory}!void {
...
fn clearTransaction(self: *WayringDataControl, cancel: bool) void {
...
fn sourceSend(context: *anyopaque, mime: []const u8, fd: std.posix.fd_t) void {
    ...
    protocol.ext_data_control_source_v1.@"send:send"(&value.resource, mime, fd) catch value.client.postOutOfMemory(&value.resource.runtime, "queueing data-control transfer");
}
fn sourceCancelled(context: *anyopaque) void {
    const value: *Source = @ptrCast(@alignCast(context));
    protocol.ext_data_control_source_v1.@"send:cancelled"(&value.resource) catch value.client.postOutOfMemory(&value.resource.runtime, "queueing data-control cancellation");
}
...
pub fn destroyClientResources(self: *WayringDataControl, client: *wayring.server.Client) void {
...
fn destroyOffer(self: *WayringDataControl, value: *Offer) void {
    self.destroyOfferResource(value, true);
}
fn destroyOfferResource(self: *WayringDataControl, value: *Offer, canonical: bool) void {
...
fn destroyDevice(self: *WayringDataControl, value: *Device) void {
...
fn destroySource(self: *WayringDataControl, value: *Source) void {
...
fn destroyManager(self: *WayringDataControl, value: *Manager) void {
...
test "EXT data-control v1 descriptors and errors are exact" {
...
const ControlFixture = struct {
    ...
    fn init(self: *@This()) !void {
        ...
        self.host.setGlobalFilter(WayringDataControl, &self.adapter, WayringDataControl.globalFilter);
        ...
        try self.bind("ext_data_control_manager_v1", 4, 1);
        try discardTestEvents(self.client());
    }
...
test "generated data-control wire owns both channels and one-closes receive descriptors" {
    ...
    try sendTestRequest(client, 4, 0, &protocol.ext_data_control_manager_v1.request_messages[0], &.{.{ .new_id = .{ .typed = 5 } }});
    try sendTestRequest(client, 4, 1, &protocol.ext_data_control_manager_v1.request_messages[1], &.{ .{ .new_id = .{ .typed = 6 } }, .{ .object = 3 } });
    try sendTestRequest(client, 4, 1, &protocol.ext_data_control_manager_v1.request_messages[1], &.{ .{ .new_id = .{ .typed = 7 } }, .{ .object = 3 } });
    try discardTestEvents(client);
    try sendTestRequest(client, 5, 0, &protocol.ext_data_control_source_v1.request_messages[0], &.{.{ .string = "text/plain" }});
    try sendTestRequest(client, 6, 0, &protocol.ext_data_control_device_v1.request_messages[0], &.{.{ .object = 5 }});
    ...
    try sendTestRequest(client, 4, 0, &protocol.ext_data_control_manager_v1.request_messages[0], &.{.{ .new_id = .{ .typed = 8 } }});
    try sendTestRequest(client, 8, 0, &protocol.ext_data_control_source_v1.request_messages[0], &.{.{ .string = "text/plain" }});
    try sendTestRequest(client, 7, 2, &protocol.ext_data_control_device_v1.request_messages[2], &.{.{ .object = 8 }});
    ...
    try sendTestRequest(client, regular_offer.resource.id(), 0, &protocol.ext_data_control_offer_v1.request_messages[0], &.{ .{ .string = "text/plain" }, .{ .fd = pipe[1] } });
    ...
    try sendTestRequest(client, 5, 0, &protocol.ext_data_control_source_v1.request_messages[0], &.{.{ .string = "text/html" }});
...
test "data-control publication is singular, atomic, and filter is restricted" {
    ...
    var adapter: WayringDataControl = undefined;
    adapter.init(std.testing.allocator, &host, undefined, undefined, undefined, 42);
    defer adapter.deinit();
    host.setGlobalFilter(WayringDataControl, &adapter, WayringDataControl.globalFilter);
    ...
    while (globals.next()) |global| if (std.mem.eql(u8, global.interface().name, "ext_data_control_manager_v1")) {
        count += 1;
    };
    ...
    var failed: WayringDataControl = undefined;
...
fn requestBind(client: *wayring.server.Client, registry: u32, global: u32, id: u32) !void {
    try sendTestRequest(client, registry, 0, &test_registry_bind, &.{
        .{ .uint = global },
        .{ .new_id = .{ .generic = .{ .interface = "ext_data_control_manager_v1", .version = 1, .id = id } } },
    });
}
