...
function M.open_file(opts)
  ...
  return with_session(function(bus)
    ...
    local results, err = portal_request(bus,
      "org.freedesktop.portal.FileChooser", "OpenFile",
      { opts.parent_window or "", opts.title or "Open File" }, options)
...
function M.save_file(opts)
  ...
  return with_session(function(bus)
    ...
    local results, err = portal_request(bus,
      ...
      { opts.parent_window or "", opts.title or "Save File" }, options)
    if not results then return nil, err end
...
function M.open_uri(uri, opts)
  ...
  return with_session(function(bus)
    ...
    local results, err = portal_request(bus,
      ...
      { opts.parent_window or "", uri }, options)
    if not results then return nil, err end
    return true
  end)
end
