...
local function acknowledge_control(device, record)
    ...
    if device.target_volume and volume and math.abs(device.target_volume - volume) < 0.0001 then
...
function Monitor:set_default(kind, name)
    ...
    local found = false
    for _, device in pairs(self.by_id) do
        if device.kind == kind and device.properties["node.name"] == name and device_availability(self, device) ~= "no" then
            found = true
            break
        end
    end
